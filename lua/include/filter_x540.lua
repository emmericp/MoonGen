...
function mod.setupRSS(dev, hash_functions, nr_queues)
  local hash_mask = ffi.new("struct mg_rss_ixgbe_hash_mask")

  for i, v in ipairs(hash_functions) do
    if (v == mod.RSS_FUNCTION_IPV4) then
      hash_mask.ipv4 = 1
    end
    if (v == mod.RSS_FUNCTION_IPV4_TCP) then
      hash_mask.tcp_ipv4 = 1
    end
    if (v == mod.RSS_FUNCTION_IPV4_UDP) then
      hash_mask.udp_ipv4 = 1
    end
    if (v == mod.RSS_FUNCTION_IPV6) then
      hash_mask.ipv6 = 1
    end
    if (v == mod.RSS_FUNCTION_IPV6_TCP) then
      hash_mask.tcp_ipv6 = 1
    end
    if (v == mod.RSS_FUNCTION_IPV6_UDP) then
      hash_mask.udp_ipv6 = 1
    end
  end

  print("lalala " .. tostring(dev.id) .. " " .. tostring(nr_queues) .. " " .. tostring(hash_mask))
  return ffi.C.mg_rss_ixgbe_setup_rss(dev.id, nr_queues, hash_mask)
end
